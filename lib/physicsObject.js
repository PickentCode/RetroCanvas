...
class PhysicsRope extends PhysicsObject {
     ...
     * @param {number} stiffness - Multiplier for the connection length from 0.1 to 10;
     * @param {number} [nodeRadius=3] - The radius of each node in the rope.
     ...
     */
    constructor(physicsSimulation, startPosition, endPosition, nodeCount, stiffness = 1, nodeRadius = 3, nodeMass = 1, collider = true, nodeName = "PhysicsNode") {
        if (!physicsSimulation) {
            throw new Error("A valid physics simulation instance is required.");
        }
        ...
        if (typeof stiffness !== "number" || stiffness < 0.1 || stiffness > 10) {
            throw new Error("Node stiffness should be a number greater or equal to 0.1 and smaller than  or equal to 10.");
        }
        ...
        for (let i = 0; i < nodeCount; i++) {
            ...
            if (i !== 0) {
                const connection = new PhysicsConnection(nodes[i-1], node, Vector.distance(nodes[i-1].position, node.position) * stiffness);
