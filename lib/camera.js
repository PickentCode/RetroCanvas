class Camera {
     ...
     * @param {boolean} smooth - If true, upscaling is smooth.
     */
    constructor(gameWidth, gameHeight, canvasWidth, canvasHeight, canvasName, smooth = false) {
        ...
        if (typeof smooth !== 'boolean') {
            throw new Error('Invalid smooth value: Expected a boolean.');
        }
    ...
    #initializeCanvas(canvasName) {
        ...
        if (!this.#smooth) this.#canvas.style.imageRendering = 'pixelated';
    ...
    #calculateFPS() {
        ...
        const fontSize = Math.max(Math.round((this.#gameWidth + this.#gameHeight) / 50), 10); // Minimum size of 10px
    ...
    /**
     * Locks the cursor on mouse click, exits with ESC.
     */
    lockCursor() {
     ...
     */
    drawAnimationFrame(frame, position, width, height, angle = 0, reflectX = false, reflectY = false) {
    ...
    drawRect(position, width, height, color = new Color(127, 0, 0, 255), angle = 0) {
        ...
        if (typeof angle !== 'number') {
            throw new Error('Invalid angle: Expected a number.');
        }
     ...
     * @param {number} [angle=0] - The rotation angle of the ellipse in degrees.
     */
    drawEllipse(center, radiusX, radiusY, color = new Color(127, 0, 0, 255), angle = 0) {
        if (!(center instanceof Vector)) {
            throw new Error('Invalid center: Expected a Vector object.');
        }
        if (typeof radiusX !== 'number' || typeof radiusY !== 'number' || typeof angle !== 'number') {
            throw new Error('Invalid arguments: radiusX, radiusY, and angle should be numbers.');
        }
        if (!(color instanceof Color)) {
            throw new Error('Invalid color: Expected a Color object.');
        }
        ...
        this.#ctx.beginPath();
        this.#ctx.ellipse(center.x, center.y, radiusX, radiusY, radianAngle, 0, 2 * Math.PI);
        this.#ctx.fillStyle = color.getColorAsHex();
        this.#ctx.fill();
    }
     ...
     * @param {number} [lineWidth=1] - The width of the outline.
     * @param {number} [angle=0] - The rotation angle of the ellipse in degrees.
     */
    drawEllipseOutline(center, radiusX, radiusY, color = new Color(127, 0, 0, 255), outlineColor = new Color(255, 0, 0, 255), lineWidth = 1, angle = 0) {
        if (!(center instanceof Vector)) {
            throw new Error('Invalid center: Expected a Vector object.');
        }
        if (typeof radiusX !== 'number' || typeof radiusY !== 'number' || typeof angle !== 'number') {
     ...
     * @param {string} [font='16px Arial'] - The font of the text.
     * @param {number} [rotationAngle=0] - The angle to rotate the text in degrees. Default is 0 (no rotation).
     * @param {boolean} [center=false] - Whether to rotate the text around its center. Default is false.
     */
    drawText(text, position, color = new Color(127, 0, 0, 255), font = '16px Arial', rotationAngle = 0, center = false) {
        ...
        if (typeof rotationAngle !== 'number') {
            throw new Error('Invalid rotation angle: Expected a number.');
        }
        if (typeof center !== 'boolean') {
            throw new Error('Invalid center value: Expected a boolean.');
        }
        ...
        if (center) {
            this.#ctx.translate(position.x, position.y); // Move to the desired position
            this.#ctx.translate(textWidth / 2, textHeight / 2); // Adjust for center rotation
            this.#ctx.rotate(rotationAngle * Math.PI / 180); // Rotate the canvas context
            this.#ctx.fillText(text, -textWidth / 2, -textHeight / 2 + textMetrics.actualBoundingBoxAscent); // Draw the text at the new origin, offsetting for the center
        } else {
            this.#ctx.translate(position.x, position.y); // Move to the desired position
            this.#ctx.rotate(rotationAngle * Math.PI / 180); // Rotate the canvas context
            this.#ctx.fillText(text, 0, textMetrics.actualBoundingBoxAscent); // Draw the text with the correct vertical alignment
        }
    ...
    /**
     * Sets the update delay for fps counter.
     * @returns {number} Delay before next update in ms.
     */
    set fpsUpdateRate(rate) {
        if (typeof rate !== 'number' || rate <= 0) {
            throw new Error('Invalid fpsUpdateRate: Expected a positive number.');
        }
        this.#fpsUpdateRate = rate;
    }
}