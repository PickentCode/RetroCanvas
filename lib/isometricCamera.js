class IsometricCamera extends Camera {
    ...
    constructor(gameWidth, gameHeight, canvasWidth, canvasHeight, canvasName, map, images, tileEnlargement = 0) {
        ...
        this.#map = map;
        this.#fogMap = Array(map.length).fill().map(() => Array(map[0].length).fill(0));
        this.#images = images;
    ...
    render() {
        ...
        for (let y = 0; y < this.#map.length; y++) {
            for (let x = 0; x < this.#map[y].length; x++) {
                ...
                if (this.#fogMap[y][x] == 1) continue;

                this.ctx.globalAlpha = 1 - this.#fogMap[y][x];
    ...
    drawObject(objX, objY, objectImage, sizeX = 1, sizeY = 1, adjustAspectRatio = false) {
        ...
        if (~~objX >= 0 && ~~objY >= 0 && ~~objX < this.#map[0].length && ~~objY < this.#map.length) {
            this.ctx.globalAlpha = 1 - this.#fogMap[~~objY][~~objX];
        }
    ...
    drawAnimation(objX, objY, animationFrame, sizeX = 1, sizeY = 1, adjustAspectRatio = false, reflectX = false, reflectY = false) {
        ...
        if (~~objX >= 0 && ~~objY >= 0 && ~~objX < this.#map[0].length && ~~objY < this.#map.length) {
            this.ctx.globalAlpha = 1 - this.#fogMap[~~objY][~~objX];
            //this.ctx.globalAlpha = this.#fogMap[~~objY][~~objX] >= 0.6 ? 0 : 1;
        }
    ...
    /**
     * Converts a canvas position to a grid position in the isometric map.
     * 
     * @param {Vector} canvasPos - The position on the canvas.
     * @returns {Vector} The corresponding grid position.
     * @throws {Error} If gridPos is not an instance of Vector.
     */
    canvasToGrid(canvasPos) {
        if (!(canvasPos instanceof Vector)) {
            throw new Error("canvasPos must be an instance of Vector.");
        }

        const { tileWidth, tileHeight, offsetX, offsetY } = this.#calculateIsometricPositioning();

        const adjustedX = (canvasPos.x - offsetX) / this.#zoomValue;
        const adjustedY = (canvasPos.y - offsetY) / this.#zoomValue;

        const gridX = (adjustedX / (tileWidth / this.#zoomValue) + adjustedY / (tileHeight / this.#zoomValue));
        const gridY = (adjustedY / (tileHeight / this.#zoomValue) - (adjustedX / (tileWidth / this.#zoomValue)));

        return new Vector(~~(gridX), ~~(gridY));
    }

    /**
     * Converts a grid position in the isometric map to a canvas position.
     * 
     * @param {Vector} gridPos - The grid position in the isometric map.
     * @returns {Vector} The corresponding canvas position.
     * @throws {Error} If gridPos is not an instance of Vector.
     */
    gridToCanvas(gridPos) {
        if (!(gridPos instanceof Vector)) {
            throw new Error("gridPos must be an instance of Vector.");
        }
    
        const { tileWidth, tileHeight, offsetX, offsetY } = this.#calculateIsometricPositioning();
    
        // Use the same calculation as in drawObject for isometric projection
        const isoX = (gridPos.x - gridPos.y) * tileWidth / 2;
        const isoY = (gridPos.x + gridPos.y) * tileHeight / 2;
    
        // The final canvas position includes the offset adjustments
        const canvasX = isoX + offsetX;
        const canvasY = isoY + offsetY;
    ...
    /**
     * Retrieves the value at a specific coordinate in the fog map.
     * Throws an error if the coordinates are out of the fog map's bounds.
     * 
     * @param {number} x - The x coordinate of the tile to retrieve.
     * @param {number} y - The y coordinate of the tile to retrieve.
     * @returns {*} The value at the given coordinates in the fog map.
     */
    getFogTile(x, y) {
        if (typeof x !== 'number' || typeof y !== 'number') {
            throw new Error("getTile: Coordinates must be numbers.");
        }
        if (x < 0 || y < 0 || y >= this.#fogMap.length || x >= this.#fogMap[y].length) {
            throw new Error("Invalid tile coordinates");
        }
    ...
    /**
     * Sets the value at a specific coordinate in the fog map.
     * Throws an error if the coordinates are out of the fog map's bounds or if the value is invalid.
     * 
     * @param {number} x - The x coordinate of the tile to set.
     * @param {number} y - The y coordinate of the tile to set.
     * @param {*} value - The value to set at the given coordinates in the fog map.
     */
    setFogTile(x, y, value) {
        if (typeof x !== 'number' || typeof y !== 'number') {
            throw new Error("getTile: Coordinates must be numbers.");
        }
        if (x < 0 || y < 0 || y >= this.#fogMap.length || x >= this.#fogMap[y].length) {
        ...
        this.#fogMap[y][x] = GameMath.clamp(value, 0, 1);
    }
    ...
    /**
     * Gets the current fog map.
     * @returns {Array} The fog map as a 2D array.
     */
    get fogMap() {
        return this.#fogMap;
    }
   ...
   /**
     ...
     */
    set map(newMap) {
        if (!Array.isArray(newMap) || !Array.isArray(newMap[0]) || !map.every(row => row.every(Number.isInteger))) {
            throw new Error('Invalid map: Expected a 2D array of integers.');
        }
        if (newMap.some(row => row.length !== newMap[0].length)) {
            throw new Error('Invalid map: Every row in the map has to have the same number of indices.');
        }
        if (newMap.every(row => row.every(index => index >= images.length))) {
            throw new Error('Invalid map: One of the indecies is higher than the number of images provided.');
        }
        this.#map = newMap;
    }
    ...
    /**
     * Sets a new fog map.
     * @param {Array} newMap - The new fog to set as the game map.
     */
    set fogMap(newMap) {
        if (!Array.isArray(newMap) || !Array.isArray(newMap[0]) || !newMap.every(row => row.every(cell => cell >= 0 && cell <= 1))) {
            throw new Error('Invalid map: Expected a 2D array of values between 0 & 1!.');
        }
        if (newMap.some(row => row.length !== newMap[0].length)) {
            throw new Error('Invalid map: Every row in the map has to have the same number of indices.');
        }
