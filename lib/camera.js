class Camera {
     ...
     */
    drawImage(image, position, width, height, angle = 0, sourcePosition = new Vector(0, 0), srcWidth = image.width, srcHeight = image.height) {
        ...
        if (sourcePosition.x < 0 || sourcePosition.y < 0 || srcWidth <= 0 || srcHeight <= 0) {
            throw new Error('Invalid arguments: sourcePosition components should be greater than or equal to 0, srcWidth, and srcHeight should be greater than 0.');
        }
        if (sourcePosition.x + srcWidth > image.width || sourcePosition.y + srcHeight > image.height) {
            throw new Error('Invalid arguments: The source rectangle should be within the bounds of the image.');
        }
        this.#ctx.save();
        this.#ctx.translate(position.x, position.y);
        this.#ctx.rotate(angle * Math.PI / 180);
        this.#ctx.drawImage(image, sourcePosition.x, sourcePosition.y, srcWidth, srcHeight, -width / 2, -height / 2, width, height);
        this.#ctx.restore();
    }
     ...
     * @param {boolean} reflectX - Reflect the sprite on the x-axis. Default is false.
     * @param {boolean} reflectY - Reflect the sprite on the y-axis. Default is false.
     */
    drawAnimationFrame(frame, position, width, height, angle = 0, reflectX = false, reflectY = false) {
        ...
        let drawX = reflectX ? -width / 2 : -width / 2;
        let drawY = reflectY ? -height / 2 : -height / 2;
