...
class PhysicsObject {
     ...
     */
    constructor(nodes, connections, name = "PhysicsObject", solidShape) {
...
class PhysicsRect extends PhysicsObject {
    ...
    constructor(physicsSimulation, position, width, height, rectName = "PhysicsRect", radius = 3, mass = 1, bounciness = 0, airFriction = 0.999, surfaceFriction = 0.75, collider = true, nodeName = "PhysicsNode") {
        ...
        physicsSimulation.addPhysicsNode(topLeft);
        physicsSimulation.addPhysicsNode(topRight);
        physicsSimulation.addPhysicsNode(bottomLeft);
        physicsSimulation.addPhysicsNode(bottomRight);
        ...
        physicsSimulation.addPhysicsConnection(topConnection);
        physicsSimulation.addPhysicsConnection(bottomConnection);
        physicsSimulation.addPhysicsConnection(leftConnection);
        physicsSimulation.addPhysicsConnection(rightConnection);
        physicsSimulation.addPhysicsConnection(diagonalConnectionTopDown);
        physicsSimulation.addPhysicsConnection(diagonalConnectionDownTop);
        ...
        super(nodes, connections, rectName, true);
    }
}
...
class PhysicsPolygon extends PhysicsObject {
    ...
    constructor(physicsSimulation, position, outerRadius, corners, polygonName = "PhysicsPolygon", nodeRadius = 3, nodeMass = 1, nodeBounciness = 0, nodeAirFriction = 0.999, nodeSurfaceFriction = 0.75, collider = true, nodeName = "PhysicsNode") {
        ...
        for (let i = 0; i < corners; i++) {
            ...
            nodes.push(node);
            physicsSimulation.addPhysicsNode(node);
        }
        ...
        for (let i = 0; i < corners; i++) {
            for (let j = i + 1; j < corners; j++) {
                ...
                physicsSimulation.addPhysicsConnection(connection);
            }
        }

        super(nodes, connections, polygonName, true);
    }
}
...
class PhysicsRope extends PhysicsObject {
    ...
    constructor(physicsSimulation, startPosition, endPosition, nodeCount, nodeRadius, nodeMass = 1, collider = true, nodeName = "PhysicsNode") {
        ...
        super(nodes, connections, "PhysicsRope", false);
    }
}