...
class PhysicsConnection {
     ...
     * @param {number} [resolveFactor=1] - The factor determining how much of the imbalance or force between the two nodes is resolved each simulation step.
     */
    constructor(node1, node2, restingDistance, collider = true, name = "PhysicsConnection", resolveFactor = 1) {
        ...
        if (typeof restingDistance !== 'number' && restingDistance != null) {
        ...
        if (typeof resolveFactor !== 'number') {
            throw new Error("resolveFactor must be a number.");
        }
        ...
        this.#resolveFactor = GameMath.clamp(resolveFactor, 0, 1);
        this.#parent = null;
    }
    ...
    solve() {
        ...
        let offset1 = new Vector(dir.x * offset * w2, dir.y * offset * w2).mult(this.#resolveFactor);
        let offset2 = new Vector(dir.x * offset * w1, dir.y * offset * w1).mult(this.#resolveFactor);
    ...
    /**
     * Gets the resolve factor of the connection.
     * @returns {number} The resolve factor.
     */
    get resolveFactor() {
    ...
    /**
     * Sets the resolve factor of the connection.
     * @param {number} value - The new resolve factor.
     */
    set resolveFactor(value) {
        if (typeof value !== 'number') {
            throw new Error("resolveFactor must be a number.");
        }
        this.#resolveFactor = value;
    }
    ...
    set parent(value) {
        if (!(value instanceof PhysicsObject) && value !== null) {
            throw new Error("Parent must be an instance of PhysicsObject or null.");
        }
        this.#parent = value;
    }
}