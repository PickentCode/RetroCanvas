...
class Input {
    ...
    #updatePosition(e) {
        ...
        let isInFullScreen = (document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement || document.msFullscreenElement) === this.#canvas;
        ...
        if (isInFullScreen) {
            ...
            if (canvasAspectRatio > screenAspectRatio) {
                ...
                scaleY = scaleX;
                offsetX = 0;
                offsetY = (window.innerHeight - (this.#canvas.height * scaleY)) / 2;
            } else {
                scaleY = rect.height / this.#canvas.height;
                scaleX = scaleY;
                offsetY = 0;
                offsetX = (window.innerWidth - (this.#canvas.width * scaleX)) / 2;
            }
    
            // Adjust the mouse position using the calculated scales and offsets
            this.#position.x = (e.clientX - rect.left - offsetX) * (1 / scaleX);
            this.#position.y = (e.clientY - rect.top - offsetY) * (1 / scaleY);
        } else {
            // Not in fullscreen, more straightforward calculation
    ...
    /**
     * Clears all the current mouse buttons presses.
     */
    clearMouseButtons() {
        this.#mouseButtons = {
            [MouseButtons.LEFT]: false,
    ...
    #attachGamepadListeners() {
        ...
        const pollGamepads = () => {
            ...
            requestAnimationFrame(pollGamepads); // Continue the loop
        };
    
        requestAnimationFrame(pollGamepads); // Start the loop
    }
    ...
    #handleTouchStart(e) {
        ...
        this.#touches = [...e.touches]; // Store the current touches
        this.onTouchStart(e);
    }

    #handleTouchMove(e) {
        e.preventDefault(); // Prevent scrolling when touching the canvas
        this.#touches = [...e.touches]; // Update the current touches
        this.onTouchMove(e);
    }

    #handleTouchEnd(e) {
        e.preventDefault(); // Prevent scrolling when touching the canvas
        this.#touches = [...e.touches]; // Update the current touches (should be empty after a touch end)
        this.onTouchEnd(e);
    }
     ...
     * @returns {TouchList} - The current touches on the screen.
     */
    getTouches() {
        return this.#touches;
    }
