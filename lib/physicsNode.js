...
class PhysicsNode {
     ...
     */
    constructor(position, radius, mass = 1, bounciness = 0, airFriction = 0.999, surfaceFriction = 0.75, locked = false, collider = true, name = "PhysicsNode") {
     ...
     */
    update(deltaTime, MAX_DELTA) {
        // Avoid rappid jumping on alt + tab
        if (deltaTime > MAX_DELTA) return;
        ...
        let velocityTerm = this.#clampVelocity(this.#maxVelocityMagnitude);
